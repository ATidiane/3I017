...
function responseConnexion(rep) {
	...
	if (json.ErrorCode == 1) {
		...
		$("#abonnements span").html(env.follows.length);
		$("#abonnes span").html(env.followers.length);
	}
	return bool;
}
...
function makeMainPanel(fromId, fromLogin, query) {
	...
	if (fromId < 0) {
		...
		if (env.fromId == env.id) {
			...
			bg_header = "<div id='bg-header'>" +
			"<span class='name-profil'>"+ fromLogin + "</span>" +
			"<input type='button' value='Ne plus suivre' class='btn-bg-header' />"+
			"</div>";
		} else if (!env.follows.has(fromId)){
			bg_header = "<div id='bg-header'>" +
			"<span class='name-profil'>"+ fromLogin + "</span>" +
			"<input type='button' value='Suivre' class='btn-bg-header'/>"+
			"</div>";
			new_message = '';
		}
	} 
...
Message.prototype.getHtml = function() {
	...
	var delete_text = replaceAll(this.text, "'", "\'");
	delete_text = replaceAll(delete_text, '"', '\"');
	var basket = '';
	...
	"<div class='one-msg'>" +
		...
		"<div id='"+div_msg_buttons+"' class='div-msg-buttons'>" +
			"<div id='"+msg_buttons+"' class='msg-buttons'>" +
				...
				"<div id='"+btn_likes+"' class='btn-likes'>" +
...
function addLikeResponse(rep,id_message){
	if(rep.ErrorCode){
		...
		alert ("Jes dfuhf");
		var btn_likes = "btn-likes-"+id_message;
		var div_unlike = "<div id='"+btn_likes+"' class='btn-likes' onclick='removeLike(this);'>" +
						"<img src=\"img/icons/unlike.png\" alt=\"like\" height=\"25\" width=\"25\"'/> " +
						"j'aime plus " +
						"</div>";
		$("#btn-likes-"+id_message).replaceWith(div_unlike);
		var nb_likes = env.msgs[id_message].likes+1;
		env.msgs[id_message].likes = nb_likes;
		var l = $("#nb-likes-"+id_message+" span").html(rep.nb_likes);
...
function removeLikeResponse(rep,id_message){
	...
	if(rep.ErrorCode){
		...
		var btn_likes = "btn-likes-"+id_message;
		var div_like = "<div id='"+btn_likes+"' class='btn-likes' onclick='addLike(this);'>" +
						"<img src=\"img/icons/like.png\" alt=\"like\" height=\"25\" width=\"25\"'/> " +
						"j'aime " +
						"</div>";
		$("#btn-likes-"+id_message).replaceWith(div_like);
		var l = $("#nb-likes-"+id_message+" span").html(rep.nb_likes);
	...
	if (env.key != undefined) {
		if(!noConnection) {
			$.ajax({
				...
				success: function(rep){
					removeLikeResponse(rep,id_message);
				}
			});
		}else {
		}
